...
export default function CheckoutResultSync({ paymentId }: CheckoutResultSyncProps) {
  ...
  useEffect(() => {
    ...
    const confirmStatus = async () => {
      ...
      if (!token) {
        if (!cancelled) {
          setSyncMessage("Faca login para atualizar o status do pedido automaticamente.");
          setHasError(true);
        }
        return;
      }
