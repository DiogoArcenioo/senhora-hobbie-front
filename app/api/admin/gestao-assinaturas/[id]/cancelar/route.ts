...
export async function POST(request: Request, context: RouteContext) {
  try {
    ...
    const backendResponse = await fetch(
      buildBackendUrl(`/admin/gestao-assinaturas/${encodedId}/cancelar`),
      {
        method: "POST",
        headers: {
    ...
    if (!backendResponse.ok) {
      const message =
        getBackendErrorMessage(payload) ?? "Nao foi possivel cancelar a assinatura.";
    ...
    return NextResponse.json(
      { message: "Erro inesperado ao cancelar a assinatura." },
      { status: 500 },
    );
  }
}
