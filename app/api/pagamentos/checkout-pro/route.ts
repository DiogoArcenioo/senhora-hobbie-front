...
export async function POST(request: Request) {
  try {
    ...
    const body = (await request.json()) as CheckoutProBody;
    ...
    const backendResponse = await fetch(buildBackendUrl("/pagamentos/checkout-pro"), {
      method: "POST",
      headers: {
    ...
    if (!backendResponse.ok) {
      const message = getBackendErrorMessage(payload) ?? "Nao foi possivel iniciar checkout no Mercado Pago.";
    ...
    return NextResponse.json(
      { message: "Erro inesperado ao iniciar checkout no Mercado Pago." },
      { status: 500 },
    );
  }
}
