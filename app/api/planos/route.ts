...
export async function GET(request: Request) {
  try {
    ...
    const backendResponse = await fetch(buildBackendUrl("/planos"), {
      method: "GET",
      headers: {
    ...
    if (!backendResponse.ok) {
      const message = getBackendErrorMessage(payload) ?? "Nao foi possivel consultar os planos.";
    ...
    return NextResponse.json(
      { message: "Erro inesperado ao consultar planos." },
      { status: 500 },
    );
  }
}
